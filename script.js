...
// Interaction functions (same as before)
...
function draw() {
  ...
  for (let j = 0; j < jars.length; j++) {
    ...
    // Add new particles (limit to 2 per frame for performance)
    if (isFilling && targetIndex === j && currentMaterial) {
      ...
      for (let i = 0; i < particlesPerFrame; i++) {
        let particle = {
          ...
          speedY: random(currentMaterial.speed * 0.8, currentMaterial.speed * 1.2) / 50, // معدل النزول النسبي
    ...
    for (let p = jarData.sparkles.length - 1; p >= 0; p--) {
      ...
      if (!s.isStatic) {
        ...
        let fillLevelY = jarBottom - jarData.fillLevel;

      
        if (currentY_abs >= fillLevelY - s.size) {
          s.isStatic = true;
