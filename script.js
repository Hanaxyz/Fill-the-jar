...
function draw() {
  ...
  for (let j = 0; j < jars.length; j++) {
    ...
    // Adding new particles - FIXED: particles fall from top
    if (isFilling && targetIndex === j && currentMaterial) {
      let particle = {
        relX: random(0.15, 0.85),
        relY: 0.0, // Start from top
        speedY: random(currentMaterial.speed * 0.5, currentMaterial.speed * 0.8), // Slower speed
    ...
    // Update fill level based on particles
    ...
    for (let p = jarData.sparkles.length - 1; p >= 0; p--) {
      ...
      if (!s.isStatic) {
        // Move down - FIXED: using delta time for smooth movement
        s.relY += s.speedY * 0.01; // Slower, smoother movement
        s.relX += s.speedX * 0.01;
        s.relX = constrain(s.relX, 0.05, 0.95);

        // Check if particle reached the fill level
        let currentY_abs = jarTop + (s.relY * jarHeight);
        
        if (currentY_abs >= stopLine - s.size) {
          s.isStatic = true;
          s.offsetFromBottom = (jarBottom - currentY_abs) / jarHeight;
          
          // Increase fill level
          if (jarData.fillLevel < maxFill) {
            jarData.fillLevel = min(jarData.fillLevel + 1.5, maxFill);
          }
        }
        
        // Remove particles that go below the jar
        if (currentY_abs > jarBottom + 50) {
          jarData.sparkles.splice(p, 1);
          continue;
        }
      }
      ...
      if (s.isStatic) {
        drawY = jarBottom - (s.offsetFromBottom * jarHeight);
      } else {
        drawY = jarTop + (s.relY * jarHeight);
      }

      // Clamp drawing position
      drawX = constrain(drawX, jarLeft + 5, jarRight - 5);
      drawY = constrain(drawY, jarTop + 5, jarBottom - 5);
...
// Logic to target a jar with pipe
function getPipeTargetIndex() {
  let pipe = document.getElementById('pipe');
  if (!pipe) return 0; // Default to first jar
