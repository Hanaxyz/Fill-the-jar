...
function draw() {
  ...
  for (let j = 0; j < jars.length; j++) {
    ...
    if (isFilling && targetIndex === j && currentMaterial) {
      ...
      let baseSpeed = currentMaterial.speed * speedFactor;
      let speedY = random(baseSpeed * 0.7, baseSpeed * 1.3);
      ...
      if (!s.isStatic) {
        ...
        if (currentY_abs >= fillLevelY - s.size) {
          ...
          if (jarData.fillLevel < maxFill) {
            jarData.fillLevel = min(jarData.fillLevel + 1.2, maxFill);
          }
        }

        if (currentY_abs > jarBottom + 100) {
