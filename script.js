...
function draw() {
  ...
  for (let j = 0; j < jars.length; j++) {
    ...
    let jarLeft = rect.left + paddingX;
    let jarRight = rect.right - paddingX;
    let jarTop = rect.top + (rect.height * 0.25);
    let jarBottom = rect.bottom - (rect.height * 0.08);
    ...
    let currentFillHeight = min(jarData.fillLevel, jarHeight * 0.85);
    let fillSurfaceY = jarBottom - currentFillHeight;
    ...
    for (let p = jarData.sparkles.length - 1; p >= 0; p--) {
      ...
      if (!s.isStatic) {
        ...
        if (s.y >= fillSurfaceY - (s.size / 2)) {
          s.isStatic = true;
          s.y = fillSurfaceY - (s.size / 2);
          
          s.offsetY = jarBottom - s.y;
          s.offsetX = s.x - jarLeft;

          if (jarData.fillLevel < maxFill) {
            jarData.fillLevel = min(jarData.fillLevel + 0.7, maxFill);
          }
        }
      }
      ...
      let drawX, drawY;
      if (s.isStatic) {
        drawX = jarLeft + s.offsetX;
        drawY = jarBottom - s.offsetY;
      } else {
        drawX = s.x;
        drawY = s.y;
      }
